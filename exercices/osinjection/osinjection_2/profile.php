<?php
  ...
  if(isset($_POST['delete']) && isset($_POST['username']) && !empty($_POST['username'])) {
    if(!isset($error) || empty($error)) {
      try {
        $cnx = new PDO('mysql:host='. $_bdd['server'] . ';dbname=' . $_bdd['database'], $_bdd['user'], $_bdd['password']);
        $cnx->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        $stmt = $cnx->prepare("DELETE FROM users WHERE username = ?");
      ...
      $userspath = "./users/";
      $extension = ".json";
      $file = $userspath . $_POST['username'] . $extension;
      system("rm " . $file);
  ...
  if(isset($_GET['userfile']) && !empty($_GET['userfile'])) {
    $userspath = "./users/";
    $extension = ".json";  
    $userfile = $userspath . $_SESSION['username'] . $extension;
    ini_set("open_basedir", $userspath);
    ...
    if(!isset($error) || empty($error)) {
      if(file_exists($userfile)) {
        if(!isset($error) || empty($error)) {
          ...
          header('Content-Disposition: attachment; filename="' . $_SESSION['username'] . $extension . '"');
          readfile($userfile);
          exit();
        }
      }
                  ...
                  ?>
                      <div class="bg-white p-3 border border-secondary rounded">
                        ...
                        <div class="text-center pt-2">
                          <?php
                          ?>
                          <a href="<?php echo 'profile.php?userfile=' . htmlspecialchars($_SESSION['username'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401); ?>">Accéder à votre profil au format JSON</a>
                        </div>
                        <form action="./profile.php" method="POST">
                          <input type="hidden" id="username" name="username" value="<?php echo htmlspecialchars($_SESSION['username'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401); ?>">
                          <div class="text-center pt-2">
                            <button type="submit" name="delete" class="btn btn-danger text-light">Supprimer mon compte</button>
                          </div>
                        </form>
                      </div>
                  <?php
