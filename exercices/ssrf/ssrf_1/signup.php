<?php
  ...
  if(isset($_POST['username']) || isset($_POST['firstname']) || isset($_POST['lastname']) || isset($_POST['password']) || isset($_POST['confirmed_password'])) {
    ...
    if(isset($_POST['avatar']) && empty($_POST['avatar'])) {
      if(!isset($error) || empty($error)) {
        header("HTTP/1.1 400 Bad Request");
        $error = "Un avatar doit être renseigné.";
      }
    }    
    ...
    if(!isset($error) || empty($error)) {
      try {
        ...
        if($user) {
          ...
          $stmt = $cnx->prepare("INSERT INTO users (username, firstname, lastname, password, role, avatar) VALUES (?, ?, ?, ?, 'user', ?)");
          $stmt->bindParam(1, $_POST["username"], PDO::PARAM_STR);
          ...
          $stmt->bindParam(5, $_POST["avatar"], PDO::PARAM_STR);
          $stmt->execute(); 
                ...
                <div class="col">
                  <div class="bg-white p-3 border border-secondary rounded">
                    <form method="POST" action="./signup.php">
                      <div class="mb-3">
                        <label for="avatar" class="form-label fw-bold text-dark">Avatar (service <a href="https://getavataaars.com/">avataars</a>) :</label>
                        <input type="text" class="form-control" id="avatar" name="avatar" value="https://avataaars.io/?avatarStyle=Circle&topType=ShortHairShortFlat&accessoriesType=Blank&hairColor=Black&facialHairType=Blank&clotheType=BlazerShirt&eyeType=Default&eyebrowType=Default&mouthType=Default&skinColor=Light">
                      </div>
                      <div class="mb-3">
