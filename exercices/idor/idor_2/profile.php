<?php
  ...
  if(isset($_POST['id']) || isset($_POST['firstname']) || isset($_POST['lastname'])) {
    if(isset($_POST['id']) && (empty($_POST['id']) || !is_numeric($_POST['id']) || intval($_POST['id']) < 1)) {
    ...
    if(!isset($error) || empty($error)) {
      try {
        ...
        $checkStmt = $cnx->prepare("SELECT COUNT(*) FROM users WHERE id = ?");
        $checkStmt->bindParam(1, $_POST["id"], PDO::PARAM_INT);
        $checkStmt->execute();
