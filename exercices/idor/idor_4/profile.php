  ...
  if(isset($_GET['phoneNumber']) && !empty($_GET['phoneNumber'])) {
    ...
    if(!isset($error) || empty($error)) {
      try {
        ...
        $stmt = $cnx->prepare("SELECT phone_number, username, firstname, lastname, role FROM users WHERE phone_number = ?");
        $stmt->bindParam(1, $_GET["phoneNumber"], PDO::PARAM_STR);
        $stmt->execute(); 
        ...
        if($_GET['phoneNumber'] !== $_SESSION['phoneNumber']) {
          if(!isset($error) || empty($error)) {
                      ...
                      <div class="bg-white p-3 border border-secondary rounded">
                        <form>
                          ...
                          <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="phoneNumber" name="phoneNumber" value="<?php echo htmlspecialchars($user['phone_number'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401); ?>" disabled>
                          </div>  
                          <div class="mb-3">
