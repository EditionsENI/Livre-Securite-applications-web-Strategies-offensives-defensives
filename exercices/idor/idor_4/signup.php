<?php
  ...
  if(isset($_POST['username']) || isset($_POST['firstname']) || isset($_POST['lastname']) || isset($_POST['password']) || isset($_POST['confirmed_password'])) {
    ...
    if(isset($_POST['phoneNumber']) && empty($_POST['phoneNumber'])) {
      if(!isset($error) || empty($error)) {
        header("HTTP/1.1 400 Bad Request");
        $error = "Le numéro de téléphone doit être renseigné.";
      }
    }
    if(isset($_POST['phoneNumber']) && !empty($_POST['phoneNumber'])) {
      if(!preg_match('/^[0-9]{10}$/', $_POST['phoneNumber'])) {
        if(!isset($error) || empty($error)) {
          header("HTTP/1.1 400 Bad Request");
          $error = "Le numéro de téléphone ne possède pas un format valide (par ex. 0156789865).";
        }
      }
    }
    ...
    if(!isset($error) || empty($error)) {
      try {
        ...
        $stmt = $cnx->prepare("SELECT * FROM users WHERE phone_number = ?");
        $stmt->bindParam(1, $_POST["phoneNumber"], PDO::PARAM_STR);
        $stmt->execute(); 
        ...
        if(!isset($error) || empty($error)) {
          ...
          $stmt = $cnx->prepare("INSERT INTO users (phone_number, username, firstname, lastname, password, role) VALUES (?, ?, ?, ?, ?, 'user')");
          $stmt->bindParam(1, $_POST["phoneNumber"], PDO::PARAM_STR);
          $stmt->bindParam(2, $_POST["username"], PDO::PARAM_STR);
          $stmt->bindParam(3, $_POST["firstname"], PDO::PARAM_STR);
          $stmt->bindParam(4, $_POST["lastname"], PDO::PARAM_STR);
          $stmt->bindParam(5, $userPassword, PDO::PARAM_STR);
          $stmt->execute(); 
                ...
                <div class="col">
                  <div class="bg-white p-3 border border-secondary rounded">
                    <form method="POST" action="./signup.php">
                      ...
                      <div class="mb-3">
                        <label for="phoneNumber" class="form-label fw-bold text-dark">Numéro de téléphone :</label>
