<?php
  ...
  if(isset($_POST['phoneNumber']) && empty($_POST['phoneNumber'])) {
    header("Location: index.php?error=" . urlencode('Le numéro de téléphone doit être renseigné.'), true, 302);
    exit();
  }
  ...
  if(isset($_POST['phoneNumber']) || isset($_POST['password'])) {
    if(isset($_POST['phoneNumber']) && !empty($_POST['phoneNumber']) && isset($_POST['password']) && !empty($_POST['password'])) {
      try {
        $cnx = new PDO('mysql:host='. $_bdd['server'] . ';dbname=' . $_bdd['database'], $_bdd['user'], $_bdd['password']);
        $cnx->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        
        $stmt = $cnx->prepare("SELECT id, phone_number, username, firstname, lastname, password, role FROM users WHERE phone_number = ?");
        $stmt->bindParam(1, $_POST["phoneNumber"], PDO::PARAM_STR);
        $stmt->execute(); 

        $user = $stmt->fetch(PDO::FETCH_ASSOC);
        if(!password_verify($_POST['password'], $user['password'])) {
          header("Location: index.php?error=" . urlencode('Le numéro de téléphone ou le mot de passe est incorrect.'), true, 302);
          exit();
        }
