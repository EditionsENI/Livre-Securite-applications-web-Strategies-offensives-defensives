<?php
  ...
  if(isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id']) && intval($_GET['id']) >= 1) {
    if(!isset($error) || empty($error)) {
      try {
        ...
        if(intval($_GET['id']) !== intval($_SESSION['id'])) {
          if(!isset($error) || empty($error)) {
  ...
  if(isset($_GET['userfile']) && !empty($_GET['userfile'])) {
    ...
    if(!isset($error) || empty($error)) {
      $userfile = $userspath . $_GET['userfile'];
      
      if(file_exists($userfile)) {
        if(!isset($error) || empty($error)) {
          header('Content-Type: application/json');
          header('Content-Disposition: attachment; filename="' . $_GET['userfile'] . '"');
          readfile($userfile);
          exit();
        }
      }
                  ...
                  ?>
                      <div class="bg-white p-3 border border-secondary rounded">
                        ...
                        <div class="text-center pt-2">
                          <a href="<?php echo 'profile.php?userfile=' . htmlspecialchars($user['username'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401) . '.json'; ?>">Accéder à votre profil au format JSON</a>
                        </div>
                      </div>
                  <?php
                    }
