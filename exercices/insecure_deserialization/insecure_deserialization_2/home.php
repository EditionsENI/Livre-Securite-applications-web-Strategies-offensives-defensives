  ...
  if(isset($_POST['username']) || isset($_POST['password'])) {
    if(isset($_POST['username']) && !empty($_POST['username']) && isset($_POST['password']) && !empty($_POST['password'])) {
      try {
        ...
        $serializedUser = new User($user['username'], $_POST['password'], $user['firstname'], $user['lastname'], $user['role']);
        $_SESSION['userObj'] = $serializedUser;

        if(isset($_POST['rememberMe']) && !empty($_POST['rememberMe']) && $_POST['rememberMe'] === "on") {
          setcookie(EXERCICE, serialize($serializedUser), time() + (365 * 24 * 60 * 60));
              ...
              <div class="d-flex align-items-center">
                ...
                <div class="col">
                  <div class="bg-white p-3 border border-secondary rounded">
                    ...
                    ?>
                      <h2>Félicitations, <?php echo htmlspecialchars($_SESSION['userObj']->firstname, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401) . ' ' . htmlspecialchars($_SESSION['userObj']->lastname, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401); ?>!</h2>
                      <p>
