  ...
  <body>
                            ...
                            <tbody>
                              <?php
                                foreach($usermessages as $usermessage) {
                                  echo "<tr>";
                                  echo "<td class='text-break'>" . htmlspecialchars($usermessage['username'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401) . "</td>";
                                  echo "<td class='message text-break'>" . htmlspecialchars($usermessage['message'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401) . "</td>";
                                  echo "</tr>";
                                }
                              ?>
                            </tbody>
    ...
    <script>
      ...
      for (var i = 0; i < messages.length; i++) {
        messages[i].innerHTML = messages[i].innerHTML.replace(/((\+\d{1,3}|\(\+\d{1,3}\)|\(\d{1,2}\))(((?:-|.)\d+){1,5}$|((?:-|.)\d+){1,5}((?:-|.))(?:-|.)\d{1,5}))|((\+\d{1,3}( )\((0)\)|(0))([1-9]([-. ]\d{2}){4}))/,
          function(match) {
